...
# no zone boundary costs
# adjusted targets:
# I need to adjust the targets so that I can directly compare it the version in
# Prioritizr. However, once I find a closely matching solution I will switch
# back to the original targets to see if it get around the same solution.
...
name = 'scenario_101_adjtargs_100K'
...
# manually change some targets to what I know should be acheivable
spec_df.loc[spec_df.name == 'hu_ot_dredgingsites', 'prop'] = 0.0
spec_df.loc[spec_df.name == 'hu_tr_vesseltraffic_d', 'prop'] = 0.28
spec_df.loc[spec_df.name == 'hu_co_fishing_dive_d', 'prop'] = 0.49
spec_df.loc[spec_df.name == 'hu_ot_underwaterinfrastructure', 'prop'] = 0.62
spec_df.loc[spec_df.name == 'hu_rf_fishing_groundfish', 'prop'] = 0.62
spec_df.loc[spec_df.name == 'hu_tr_portsandterminals', 'prop'] = 0.64
spec_df.loc[spec_df.name == 'hu_rf_fishing_crab', 'prop'] = 0.64
spec_df.loc[spec_df.name == 'hu_ot_citypopulation_d', 'prop'] = 0.65
spec_df.loc[spec_df.name == 'hu_rf_fishing_prawnandshrimp', 'prop'] = 0.68
spec_df.loc[spec_df.name == 'hu_ot_floatingstructures', 'prop'] = 0.69
